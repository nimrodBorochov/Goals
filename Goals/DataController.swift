...
class DataController: ObservableObject {
    ...
    // Make it singleton so we don't  load it more then one time
    ...
    func createSampleData() {
        ...
        for tagCounter in 1...5 {
            ...
            tag.id = UUID()
            tag.name = String.localizedStringWithFormat(
                NSLocalizedString("Tag %d", comment: "Create a Sample Data"),
                tagCounter
            )

            for goalCounter in 1...10 {
                let goal = Goal(context: viewContext)
                goal.title = String.localizedStringWithFormat(
                    NSLocalizedString("Goal %d-%d", comment: "Create a Sample Data"),
                    tagCounter, goalCounter
                )
                goal.content = NSLocalizedString("Description goes here", comment: "Create a Sample Data")
                goal.creationDate = .now
                goal.completed = Bool.random()
                goal.priority = Int16.random(in: 0...2)
                tag.addToGoals(goal)
            }
        }
    ...
    func deleteAll() {
        ...
        let request2: NSFetchRequest<NSFetchRequestResult> = Goal.fetchRequest()
        delete(request2)

        save()
    }

    func missingTags(from goal: Goal) -> [Tag] {
        let request = Tag.fetchRequest()
        let allTags = (try? container.viewContext.fetch(request)) ?? []

        let allTagsSet = Set(allTags)
        let difference = allTagsSet.symmetricDifference(goal.goalTags)

        return difference.sorted()
    }

    /// Runs a fetch request with various predicates that filter the user's goals based
    /// on tag, title and content text, search tokens, priority, and completion status.
    /// - Returns: An array of all matching goals.
    func goalsForSelectedFilter() -> [Goal] {
    ...
    func newGoal() {
        ...
        goal.title = NSLocalizedString("New goal", comment: "Create a new goal")
        goal.creationDate = .now
        goal.priority = 1

        // If we're currently browsing a user-created tag, immediately
        // add this new goal to the tag otherwise it won't appear in
        // the list of goals they see.
    ...
    func hasEarned(award: Award) -> Bool {
        ...
        case "goals":
            // return true if they added a creation number of goal
            let fetchRequest = Goal.fetchRequest()
            let awardCount = count(for: fetchRequest)
            return awardCount >= award.value

        case "closed":
            // return true if they closed a creation number of goal
            let fetchRequest = Goal.fetchRequest()
        ...
        case "tags":
            // return true if they created a creation number of tags
